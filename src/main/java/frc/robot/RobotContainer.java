...
import frc.robot.Commands.singlemotion.alignToAprilTag;
...
public class RobotContainer {
  ...
  private void configureButtonBindings() {
    ...
    new JoystickButton(m_driverController, 2).onTrue(new InstantCommand(() -> {
      new AutoBuilder.followPath(paths.path).schedule();
    }));
  ...
  public Command getAutonomousCommand() {
    try {
      ...
      PathPlannerPath path = PathPlannerPath.fromPathFile("prueba");
